...
export function EditTransactionModal({ transaction, isOpen, onClose, onSave }: EditTransactionModalProps) {
  ...
  const { categoryNames, categoryIcons, getExpenseSubcategories, incomeSubcategories } = useDynamicCategories();
  ...
  useEffect(() => {
    if (transaction && isOpen) {
      ...
      if (transaction.type === 'expense') {
        setAvailableSubcategories(getExpenseSubcategories(transaction.category));
      } else {
  ...
  const formatCurrencyInput = (value: string) => {
    ...
    if (numbers) return new Intl.NumberFormat('id-ID').format(parseInt(numbers));
    return '';
  };
  ...
  const handleClose = () => { reset(); setAmountDisplay(''); onClose(); };
        ...
        <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4">
          <div className="space-y-2">
            ...
            <Select value={selectedType} onValueChange={(value: 'income' | 'expense') => {
              ...
              <SelectContent>
                <SelectItem value="income"><span className="flex items-center gap-2 text-success">💰 Pemasukan</span></SelectItem>
                <SelectItem value="expense"><span className="flex items-center gap-2 text-destructive">💸 Pengeluaran</span></SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          {selectedType === 'expense' && (
            <div className="space-y-2">
              ...
              <Select value={selectedCategory} onValueChange={(value) => { setValue('category', value); setValue('subcategory', ''); }}>
          ...
          <div className="space-y-2">
            ...
            <Select value={selectedSubcategory} onValueChange={(value) => setValue('subcategory', value)} disabled={selectedType === 'expense' && !selectedCategory}>
          ...
          <div className="space-y-2">
            ...
            <Textarea value={watch('description') || ''} onChange={(e) => setValue('description', e.target.value)} placeholder="Catatan tambahan" className="resize-none" rows={2} />
